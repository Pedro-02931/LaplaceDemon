...
apply_zram() {
    echo "Stopping zram-swap" >> "$LOGFILE"
    sudo systemctl stop zram-swap.service >> "$LOGFILE" 2>&1

    sudo sed -i "s/^comp_algorithm=.*/comp_algorithm=$ZRAM_ALGO/" /etc/systemd/zram.conf >> "$LOGFILE" 2>&1

    echo "Starting zram-swap" >> "$LOGFILE"
    sudo systemctl start zram-swap.service >> "$LOGFILE" 2>&1
}
...
echo "🚀 Iniciando motor de cruzamento bayesiano..." >> "$LOGFILE"
...
while true; do
    ...
    echo -e "\n🔁 $(date) :: Chave: $POLICY_KEY :: Média CPU: ${AVG_CPU}%" >> "$LOGFILE"
    echo -e "📦 Configuração de cruzamento:\n$POLICY" >> "$LOGFILE"

    read -r CPU_GOV EPB POWER_LIMIT TDP_LIMIT GPU_CLOCK ZRAM_ALGO CORES SWAPPINESS <<< "$POLICY"
